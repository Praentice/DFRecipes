...
sudo pacman -S vsftd nginx dnsmasq #Alternative for arch based system
```
...
```
And you should be able to brose the directory within the web server.
#### Add services
...
sudo systemctl disable vsftpd #Allow the service vsftp to start on the machine startup
